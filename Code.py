...
endInvertedIndexTime = time()

#Time for making the inverted index table
print("\nThe time taken to make the inverted index table is: ", round(endInvertedIndexTime-startInvertedIndexTime, 4), "seconds\n")
...
while(True):
    if(choice == '1'):
        ...
        startPreProcessingTime = time()
        preProcessedQuery = QueryPreProccess(query)
        endPreProcessingTime = time()
        
        print("The query being run is: ", preProcessedQuery)
        print("\nThe time taken to preprocess the query is: ", round(endPreProcessingTime-startPreProcessingTime, 4), " seconds")
        
        # parsing thru the complete query
        startTimeRetrieveDocs = time()
        ...
        endTimeRetrieveDocs = time()
        print("\nThe time taken to retrieve the relevant documents is: ", round(endTimeRetrieveDocs-startTimeRetrieveDocs, 4), " seconds")
        print('\n')
